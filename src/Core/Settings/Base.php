...
/**
 * Class Bloks_Core_Settings_Base
 *
 * @package     Bloks
 * @subpackage  Settings\Base
...
abstract class Bloks_Core_Settings_Base
{
    ...
    public function enqueueScripts()
    {
        if (is_admin() && isset($_GET['page'])
            && sanitize_text_field($_GET['page']) == 'bloks_options'
        ) {
    ...
    public function sanitize($data)
    {
        ...
        foreach ($data as $input => $value) {
            foreach ($this->items as $group) {
                if(isset($group['items'][$input])) {
                    $instance = $group['items'][$input]['instance'];
                    $new_input[$input] = $instance->sanitize($value);
                }
            }
        }
